...
Approach:- DP
    ...
    3. recursive solution using include and exclude
...
int solve2(vector<int>& coins, int n, int amount, vector<vector<int>> &dp)
{
    ...
    int exclude = 0 + solve2(coins, n-1, amount, dp);
    
    int include = INT_MAX;
    if(coins[n]<=amount)
        include = 1 + solve2(coins, n, amount-coins[n], dp);
...
int coinChange2(vector<int>& coins, int amount)
{
    ...
    int ans = solve2(coins, i-1, amount, dp);
