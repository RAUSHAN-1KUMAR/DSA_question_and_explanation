...
Approach:- Better approach using list
    -- using list would give TC: O(1) for deletion of a node
...
{
    int cap;
    list<int> dll; // back->needs to be removed, front->most recently used
    unordered_map<int, pair<list<int>::iterator, int>> cache; // {key -> {node, value}}

    void moveTheKeyRecentlyUsed(int key)
    {
