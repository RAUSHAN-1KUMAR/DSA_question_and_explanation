...
Approach:- explained in notecopy
    -- used (mid&1) in place of (mid%2==1)

Note:- We can use XOR of each element to find that odd occurence element but the time complexity will be O(n): to reduce the time complexity we will use binary search.
*/
...
int binarySearch(int arr[], int n)
{
    ...
    while(s<=e)
    {
        ...
        // if mid is odd or even
        if(mid&1){ //mid&1 -> true, it means odd number
