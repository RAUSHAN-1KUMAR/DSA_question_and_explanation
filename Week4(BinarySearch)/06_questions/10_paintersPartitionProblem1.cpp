...
int isPossibleSolution(int A[], int N, int M, int sol)
{
    int totalTime=0;
    int painter=1;
    ...
    for(int i=0 ; i<N ; i++)
    {
        // if you don't want to write this condition, then start the start form max of A[] not from 1
        if(A[i]>sol) return INT_MAX;// important condition
...
long long minTime(int A[], int N, int M) 
{
    ...
    while(start<=end)
    {
        ...
        if(isPossibleSolution(A, N, M, mid)<=M)
        {
            ans=mid;
            end=mid-1;
        }
