...
/*
Appraoch:- search space + predicate function
    - question is similar to aggressive cow problem
*/
...
bool isPossibleSolution(vector<int> &position, int mid, int m)
{
    int count = 1;
...
int maxDistance(vector<int> &position, int m)
{
    ...
    while (start<=end)
    {
        ...
        if(isPossibleSolution(position, mid, m)){
            ans = mid;
            start = mid+1;
        }
        else{
