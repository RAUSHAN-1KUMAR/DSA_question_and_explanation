...
For example, consider the following code:
    ...
    // assigning address to elements in the array of pointers
    arr[0] = &nums[0];
    arr[1] = &nums [1];
    arr[2] = &nums [2]; 
...
int (*ptr) [10] is a pointer to an array of 10 integers in C++. This means that ptr is a pointer that points to the first element of an array that contains 10 integers.
    ...
    In the above example, nums is an array of 10 integers, and ptr is a pointer to the first element of nums. The parentheses around *ptr are necessary because the dereference operator has a lower precedence than the array subscript operator [].
