...
Here's an example of how to declare a void pointer:
    ...
    2. To use a void pointer in C++, we must first cast it to a specific data type. For example, if we want to use a void pointer to point to an integer, we can cast it to an int* data type:
        void* ptr;
        int x = 10;
        ptr = &x;
    ...
    3. Void pointers are commonly used in C++ for dynamic memory allocation using the new operator. When we allocate memory dynamically using new, the type of the allocated memory is determined at runtime. We can use a void pointer to point to the dynamically allocated memory and then cast it to the appropriate data type when we need to use it.
        int* ptr = new int;
        void* voidPtr = ptr;
...
// Generic print function using void pointers
...
int main() {
    ...
    // we can observe that we are passing address of different data types into a same function
    print(&i, 'i'); 
    print(&f, 'f'); 
    print(&c, 'c');
