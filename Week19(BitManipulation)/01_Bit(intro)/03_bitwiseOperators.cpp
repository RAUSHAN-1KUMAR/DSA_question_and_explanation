/*
Bitwise operators 
    Ex-> bitwise compliment(~), left shift(<<), right shift(>>), bitwise OR(|), bitwise AND(&), bitwise exclusive OR(^)-> XOR
...
int main()
{
    ...
    cout << (num1<<1) << endl;//-> as discussed multiply 5*(2^1) i.e., 10
    cout << (num1>>1) << endl;//-> as discussed divide 5/(2^1) i.e., 2
...
--> Bitwise compliment for positive number:- Ex:- ~5 that is bitwise compliment of -6
    ...
    5. Now to obtain the number(as output, means in decimal form), we take 2's compliment of above obtained 1's compliment because MSB is 1 i.e., for compiler it's a negative number now
